...
# --- Định nghĩa hàm trích xuất thời gian ---
def tx_khung(x_search, y_search, x2_search, y2_search, left_margin_ratio, right_margin_ratio, top_margin_ratio, bottom_margin_ratio):
    """
    Hàm trích xuất thời gian dạng hh:mm:ss từ một vùng màn hình đã được tìm thấy bằng khung mẫu.
    
    Returns:
        tuple: (tổng số giây, ảnh đã cắt)
    """
    ...
    try:
        ...
        template_path = os.path.join(base_dir, "buttons", "khung_ts.png")
        ...
        if max_val > 0.6:  # Sử dụng ngưỡng độ chính xác
            ...
            # Tính toán các lề cố định để cắt ảnh
            ...
            # Cắt ảnh từ vùng tìm kiếm ban đầu
            crop_img = search_region_img.crop((x1_crop, y1_crop, x2_crop, y2_crop))
            
            # Trích xuất toàn bộ chuỗi ký tự, sử dụng psm 6 để đọc một dòng văn bản
            # Cấu hình 'psm 6' phù hợp để đọc một dòng số và ký tự
            text = pytesseract.image_to_string(
                crop_img,
                config='--psm 6'
            ).strip()
            
            # Lọc chuỗi và phân tích thời gian
            ...
            print(f"Chuỗi OCR: '{text}', Tổng giây: {tong_giay}")
            return tong_giay, crop_img
...
# --- Bỏ hàm tx_khung_thoi_gian cũ, chỉ sử dụng hàm tx_khung đã sửa đổi ---
...
def hien_popup(img, text_extracted):
    ...
    label_result = tk.Label(popup, text=f"Thời gian trích xuất: '{text_extracted}'", font=("Arial", 12))
