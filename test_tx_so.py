...
def tx_so(bbox, crop_ratio=(0.2, 0.2, 0.2, 0.2)):
    """
    Trích xuất số từ một vùng màn hình đã được định nghĩa bởi bbox, với tỷ lệ cắt tùy chỉnh.
    Đây là phiên bản độc lập của hàm tx_so.
    ...
    try:
        ...
        # Trích xuất số, sử dụng psm 7 để đọc một ký tự hoặc một từ duy nhất
        text = pytesseract.image_to_string(crop_img, config='--psm 7 digits').strip()
        
        # Chuyển đổi kết quả sang số nguyên
        try:
            so_trich_xuat = int(''.join(filter(str.isdigit, text)))
        except ValueError:
            so_trich_xuat = 0

        print(f"OCR='{text}', số đã trích xuất={so_trich_xuat}")
        return so_trich_xuat, crop_img

    except Exception as e:
        print(f"Lỗi trong hàm tx_so: {e}")
        return 0, None
...
def process_and_display():
    ...
    so_trich_xuat, img_cropped = tx_so(bbox, crop_ratio)
